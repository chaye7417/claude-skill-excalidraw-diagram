...
description: 从文本内容生成 Excalidraw 图表用于 Obsidian。当用户要求创建图表、流程图、思维导图或 Excalidraw 格式的可视化表示时使用。触发词：「Excalidraw」、「画图」、「流程图」、「思维导图」、「可视化」、「diagram」。
metadata:
  version: 1.1.0
---
...
从文本内容创建 Excalidraw 图表，输出 Obsidian 可用的 `.md` 文件。
...
4. 生成带 Excalidraw frontmatter 的 Obsidian `.md` 文件
5. **自动保存到当前工作目录**
6. 通知用户文件路径并确认保存成功
...
所有元素类型请参见 [references/excalidraw-schema.md](references/excalidraw-schema.md)。
